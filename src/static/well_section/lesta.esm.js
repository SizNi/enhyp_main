...
// packages/lesta/create/app/renderComponent.js
function renderComponent(nodeElement, component2, section, hasHTML) {
    ...
    if (proxies) {
      ...
      for (const key in proxies) {
        ...
        if (this.props.proxies && key in this.props.proxies) {
          ...
          const storeModule = await this.context.store.get(store2);
          if (!storeModule)
...
function sections_default(propertyComponent, specialty, nodeElement, proxies, create) {
  if (propertyComponent.sections) {
    const mount = async (section, options) => {
      if (propertyComponent.iterate)
...
var Components = class extends Node {
  ...
  async create(specialty, nodeElement, propertyComponent, proxies, value, index) {
    if (!propertyComponent.src)
      return errorComponent(nodeElement.nodepath, 203);
    const { src, abortSignal, aborted } = propertyComponent;
    let container = null;
    if (!nodeElement.process) {
      nodeElement.process = true;
      container = await this.app.mount({
        src,
        abortSignal,
        aborted,
        container: nodeElement,
        ...props_default.collect(propertyComponent, proxies, value, index)
      });
      delete nodeElement.process;
    }
    if (!container)
      return;
    sections_default(propertyComponent, specialty, container, (proxies2, target, section) => {
      if (index !== void 0) {
...
var Basic = class extends Components {
  ...
  async init() {
    const mount = async () => await this.create(this.proxies.bind(this), this.nodeElement, this.node.component, this.proxies(this.node.component.proxies, this.nodeElement));
    if (this.node.component.induce) {
      if (typeof this.node.component.induce !== "function")
        return errorComponent(this.nodeElement.nodepath, 212);
      this.impress.collect = true;
...
function createApp(app = {}) {
  app.initPlugin = (plugin, options) => plugin.install(app, options), app.mount = async ({ src, signal, aborted, container, params, methods, proxies, section }) => {
    ...
    component2.context.options.inputs = { params, methods, proxies, section };
    const hasHTML = app.router?.to.route.static;
    const render = () => renderComponent(container, component2, section, hasHTML);
...
var RouterBasic = class {
  constructor(options) {
    ...
    collectorRoutes_default(this.routes, this.router.collection);
  }
  initBasic(app) {
  ...
  async afterHooks(hook) {
    if (hook)
      await hook(this.router.to, this.router.from, this.plugins);
  }
  async update(url) {
    let res = null;
    if (await this.beforeHooks(this.beforeEach))
      return;
    const to = route_default.init(this.router.collection, url);
...
var Router = class extends RouterBasic {
  constructor(...args) {
    super(...args);
...
function createRouter(options) {
  ...
  return {
    init: router2.init.bind(router2),
    link: router2.link.bind(router2),
    push: router2.push.bind(router2)
  };
}
export {
  createApp,
